...
a) 光影一致性 (Lighting Integration)：产品的光源方向、阴影方向是否与场景中其他物体一致？产品高光/反射是否匹配环境光？如果场景是侧光但产品看起来是正面打光，判定为不融合。**当画面中存在多个同款产品时，每一个产品都必须逐一检查光影方向，不能只看主体，任何一个融合失败均判定为 WARN 或 FAIL。**
...
d) 接触面物理性 (Contact Realism)：产品与承载面的接触是否自然？是否有悬浮感、缺少阴影、或底部切割不自然？**同时检查产品各部件之间的接触：盖子必须放置在瓶口/容器口上，附件（如学位帽、把手、流苏）必须与主体有明确接触点，不能悬浮在主体上方空中——此类情况判定为 FAIL。**
...
b) 画面崩坏：是否存在明显的 AI 生成痕迹（如：局部严重模糊重影、不合理的纹理重复）。**特别注意：产品周围是否出现不合理的放射状光束、异常光晕或光圈（如产品底部/周围出现向四周发散的光线，而场景中其他物体没有此现象）——这类异常光效是严重的 AI 幻觉，判定为 FAIL。**

**维度 5：字幕排版建议**
分析生成图的画面构图，给出字幕放置建议：
- 判断画面的视觉重心和空白区域
- 建议字幕放在不遮挡产品/模特关键部位的位置
- **首先检查画面中是否存在文字、横幅、标语、标牌等元素（如派对横幅、祝贺标语、品牌文字），字幕推荐位置必须避免与这些元素重叠，否则字幕将难以辨认。**
...
a) 光影一致性：产品的光源方向、阴影是否与场景环境匹配？**当画面中存在多个同款产品时，每一个产品都必须逐一核查光影，任何一个融合失败均判定 WARN/FAIL。**
b) 比例合理性：产品相对于场景中人物、家具的大小是否符合物理常识？
c) 透视与景深：产品的透视角度是否与场景匹配？
d) 接触面物理性：产品与承载面的接触是否自然（无悬浮感）？**同时检查产品各部件之间的接触：附件（如学位帽、盖子、把手、流苏）必须与主体有明确接触点，不能悬浮在主体上方空中，否则判定为 FAIL。**
...
b) 画面崩坏：明显 AI 生成痕迹（模糊重影、纹理重复）？**特别注意：产品周围是否出现不合理的放射状光束、异常光晕（产品底部/周围向四周发散的光线，而场景其他物体没有此现象）——此类异常光效判定为 FAIL。**

**维度 4：字幕排版建议**
**首先检查画面中是否存在文字、横幅、标语、标牌等元素，字幕推荐位置必须避免与这些元素重叠。**
可选位置：top_left / top_center / top_right / bottom_left / bottom_center / bottom_right
...
**维度 2：场景融合度**
a) 光影一致性：产品光源方向与场景是否匹配？**当画面中存在多个同款产品时，每一个产品都必须逐一核查光影，任何一个融合失败均判定 WARN/FAIL。**
b) 比例合理性：产品相对于场景人物/家具的大小是否合理？
c) 透视与景深：透视角度是否与场景一致？
d) 接触面物理性：产品与承载面接触是否自然（无悬浮感）？**同时检查产品各部件之间的接触：附件（如学位帽、盖子、把手、流苏）必须与主体有明确接触点，不能悬浮在主体上方空中，否则判定为 FAIL。**
...
b) 画面崩坏：是否有明显 AI 生成痕迹？**特别注意：产品周围是否出现不合理的放射状光束、异常光晕（如产品底部/周围向四周发散的光线，而场景其他物体没有此现象）——此类异常光效判定为 FAIL。**
